    ...
    public static class CalendarGenerator
    {
        public static List<CalendarDayData> Generate(DateTime month, GroupData group, Dictionary<string, DayOverrideData> overrides)
        {
            ...
            for (int i = 0; i < 42; i++)
            {
                ...
                d.shiftName = ShiftStyleUtility.GetName(d.ResolvedShift, group?.shiftTypes);
                d.shiftColorHex = ShiftStyleUtility.GetColorHex(d.ResolvedShift, group?.shiftTypes);
                if (group?.shiftTypes != null)
                {
                    ShiftCal.Data.ShiftTypeDefinitionData definition = group.shiftTypes.Find(item => item != null && item.id == d.ResolvedShift);
                    if (definition != null)
                    {
                        d.startTime = definition.startTime;
                        d.endTime = definition.endTime;
                        d.hours = definition.hours;
                    }
                }
